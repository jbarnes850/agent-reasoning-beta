...
async def test_reasoning_engine(mock_env, mock_config):
    ...
    response = AgentResponse(
        content="Test response", confidence=Confidence.HIGH, metadata={"test": "data"}
    )
...
async def test_agent_response_validation():
    ...
    config = ModelConfig(
        name="test", provider="test", model_id="test-model", temperature=0.7
    )
    engine = ReasoningEngine(config)
    ...
    # Test empty content
    with pytest.raises(ValueError):
        response = AgentResponse(content="", confidence=Confidence.LOW, metadata={})
