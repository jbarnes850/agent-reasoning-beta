...
class ReasoningEngine:
    ...
    async def _simulate(self, node: MCTSNode, agent_state: AgentState) -> float:
        ...
        while depth < self.max_depth:
            ...
            new_thought = await self._generate_thought(current_node.thought, agent_state)
    ...
    async def _generate_thought(self, parent_thought: ThoughtNode, agent_state: AgentState) -> ThoughtNode:
    ...
    async def _call_anthropic(self, prompt: str, agent_state: AgentState) -> ModelResponse:
    ...
    # Prompt templates
    def _build_thought_prompt(self, parent_thought: ThoughtNode, agent_state: AgentState) -> str:
...
Next thought:"""
    ...
    def _build_verification_prompt(self, path: ReasoningPath) -> str:
        ...
        thoughts = "\n".join([f"{i+1}. {t.content}" for i, t in enumerate(path.thoughts)])
...
3. Detailed feedback"""
    ...
    async def verify_path(self, path: ReasoningPath, agent_state: AgentState) -> VerificationResult:
    ...
    async def build_consensus(
        ...
        verifications = await asyncio.gather(*[
            self.verify_path(path, state)
            for path, state in zip(paths, agent_states)
        ])
        
        # Filter valid paths
        valid_paths = [
            v.path for v in verifications
        ...
        best_path = max(
            valid_paths,
            key=lambda p: sum(t.confidence for t in p.thoughts)
        )
        
        return ConsensusResult(
            paths=paths,
            consensus_path=best_path,
            confidence=sum(t.confidence for t in best_path.thoughts) / len(best_path.thoughts),
            participating_agents={state.id for state in agent_states}
        )
    ...
    def _extract_best_path(self, root: MCTSNode) -> ReasoningPath:
        ...
        while current.children:
            ...
            best_child_id = max(
                current.children,
                key=lambda x: self.mcts_nodes[x].mean_value
            )
