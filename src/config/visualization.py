...
class TreeConfig(BaseVisualizationConfig):
    ...
    NODE_SIZE_RANGE: Tuple[int, int] = Field(
        default=(20, 50),
        description="Range for node sizes (min, max)"
    )
    EDGE_WIDTH_RANGE: Tuple[int, int] = Field(
        default=(1, 5),
        description="Range for edge widths (min, max)"
    )
...
class VisualizationConfig(BaseModel):
    ...
    tree: TreeConfig = Field(
        default_factory=TreeConfig,
        description="Tree visualization configuration"
    )
    graph: GraphConfig = Field(
        default_factory=GraphConfig,
        description="Graph visualization configuration"
    )
    metrics: MetricsConfig = Field(
        default_factory=MetricsConfig,
        description="Metrics visualization configuration"
    )
