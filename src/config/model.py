...
class GroqConfig(ModelConfig):
    ...
    provider: Literal[ModelProvider.GROQ] = Field(
        default=ModelProvider.GROQ, description="Groq model provider"
    )
    model_name: str = Field(
        default="llama-3.1-70b-versatile", description="Default Groq model"
    )
...
class OpenAIConfig(ModelConfig):
    ...
    provider: Literal[ModelProvider.OPENAI] = Field(
        default=ModelProvider.OPENAI, description="OpenAI model provider"
    )
    model_name: str = Field(default="gpt-4o", description="Default OpenAI model")
...
class AnthropicConfig(ModelConfig):
    ...
    provider: Literal[ModelProvider.ANTHROPIC] = Field(
        default=ModelProvider.ANTHROPIC, description="Anthropic model provider"
    )
    model_name: str = Field(
        default="claude-3-5-sonnet-latest", description="Default Anthropic model"
    )
