...
class ConfigManager:
    ...
    def load_config(self, env: str = None) -> AppConfig:
        ...
        try:
            # Load default config
            default_config = self._load_json_file(self.config_dir / self.DEFAULT_CONFIG_PATH)
            
            # Load environment-specific config
            env_config_path = self.config_dir / f"{self.ENV_CONFIG_PREFIX}/{env}.json"
            env_config = self._load_json_file(env_config_path) if env_config_path.exists() else {}
