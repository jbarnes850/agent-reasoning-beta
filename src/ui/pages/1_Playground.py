...
def render_playground():
    ...
    st.markdown(
        "<h1 class='main-header'>🎮 Agent Reasoning Playground</h1>",
    ...
    with col1:
        st.markdown(
            "<h2 class='section-header'>🔍 Experiment Visualization</h2>",
            unsafe_allow_html=True,
        )
    ...
    with col2:
        st.markdown(
            "<h2 class='section-header'>⚙️ Configuration</h2>", unsafe_allow_html=True
        )
        ...
        with st.form("experiment_config"):
            ...
            submitted = st.form_submit_button(
                "🚀 Start Experiment", use_container_width=True, type="primary"
            )
    ...
    st.markdown(
        "<h2 class='section-header'>📊 Experiment Status</h2>", unsafe_allow_html=True
    )
...
def display_status():
    ...
    if st.session_state.get("experiment_running", False):
        ...
        st.write(
            "Current Phase:", st.session_state.get("current_phase", "Initializing")
        )
            ...
            with st.expander(f"Agent {agent.id}"):
                ...
                with col1:
                    ...
                    st.metric(
                        ...
                        agent.metrics.successful_verifications,
                    )
                with col2:
                    st.metric(
                        "Failed Verifications", agent.metrics.failed_verifications
                    )
                    st.metric(
                        "Consensus Participations",
                        agent.metrics.consensus_participations,
                    )
    else:
