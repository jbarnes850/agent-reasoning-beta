...
def render_analytics():
    ...
    with col4:
        st.metric(
            "Total Cost", f"${metrics_manager.get_total_cost(start_date, end_date):.2f}"
        )

    # Detailed metrics
    tab1, tab2, tab3 = st.tabs(
        ["Performance Analysis", "Agent Interactions", "Resource Usage"]
    )
    ...
    with tab1:
        ...
        success_rates = metrics_manager.get_success_rate_distribution(
            start_date, end_date
        )
        fig = px.histogram(
            success_rates, x="success_rate", title="Success Rate Distribution"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        interaction_patterns = metrics_manager.get_interaction_patterns(
            start_date, end_date
        )
        fig = px.scatter(
    ...
    with tab3:
        ...
        # Cost trends
        fig = px.line(
            costs["daily_costs"], x="date", y="cost", title="Daily Cost Trends"
        )
        st.plotly_chart(fig, use_container_width=True)

    # Export options
    st.sidebar.header("Export Options")
    export_format = st.sidebar.selectbox("Export Format", ["CSV", "JSON", "PDF"])
...
def export_analytics(
    ...
    if format == "CSV":
        st.download_button("Download CSV", data.to_csv(), "analytics.csv", "text/csv")
    elif format == "JSON":
        st.download_button(
            "Download JSON", data.to_json(), "analytics.json", "application/json"
        )
    else:  # PDF
        st.download_button(
            "Download PDF",
