...
from src.config.agent import AgentConfig, SystemConfig
...
def render_settings():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Model Providers", "Agent Settings", "System Settings", "Search Settings"]
    )
...
def render_model_settings(config: SystemConfig):
    ...
    with col2:
        ...
        tavily_key = st.text_input(
            "Tavily API Key", value=config.search_config.api_key or "", type="password"
        )
        if tavily_key:
...
def render_agent_settings(config: SystemConfig):
    ...
    config.max_agents = st.number_input(
        "Maximum Concurrent Agents", min_value=1, max_value=50, value=config.max_agents
    )
...
def render_system_settings(config: SystemConfig):
    ...
    config.visualization_layout = st.selectbox(
        ...
        index=["Tree", "Force-Directed", "Hierarchical"].index(
            config.visualization_layout
        ),
    )
    ...
    config.show_confidence = st.checkbox(
        "Show Confidence Scores", value=config.show_confidence
    )

    config.realtime_updates = st.checkbox(
        "Real-time Updates", value=config.realtime_updates
    )
    ...
    config.max_cpu_percent = st.number_input(
        "Max CPU Usage (%)", min_value=10, max_value=100, value=config.max_cpu_percent
    )
...
def render_search_settings(config: SystemConfig):
    ...
    search_config.include_news = st.checkbox(
        "Include News Sources", value=search_config.include_news
    )

    search_config.include_academic = st.checkbox(
        "Include Academic Sources", value=search_config.include_academic
    )
    ...
    with st.expander("Custom Search Settings"):
        search_config.max_depth = st.slider(
            "Max Search Depth", min_value=1, max_value=10, value=search_config.max_depth
        )
