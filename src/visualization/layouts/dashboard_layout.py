...
class DashboardLayout:
    ...
    def render_performance_metrics(self):
        ...
        with col2:
            ...
            if confidence_data:
                fig = px.histogram(
                    confidence_data, x="value", title="Confidence Score Distribution"
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
    ...
    def render_cost_tracking(self):
        ...
        if any(costs.values()):
            ...
            for provider, cost in costs.items():
                st.metric(f"{provider.title()} Cost", f"${cost:.4f}")
        else:
    ...
    def render_error_analysis(self):
        ...
        if errors:
            ...
            for error_type, count in errors.items():
                st.metric(error_type, count, delta=None)
        else:
    ...
    def render_response_times(self):
        ...
        if times:
            # Response time trend
            fig = px.line(times, x="timestamp", y="value", title="Response Time Trend")
            st.plotly_chart(fig, use_container_width=True)
