        ...
        with st.sidebar:
            ...
            provider = st.selectbox(
                ...
                index=["groq", "openai", "anthropic"].index(
                    self.session.model_state.provider
                ),
                format_func=lambda x: {
                    "groq": "🚀 Groq",
                    "openai": "🌟 OpenAI",
            ...
            model = st.selectbox("Model", options=models[provider], key="model_name")

            if (
                provider != self.session.model_state.provider
                or model != self.session.model_state.model
            ):
    ...
    def render_visualization_controls(self):
        ...
        with col1:
            pan_x = st.number_input(
                "Pan X", value=self.session.viz_state.pan_position[0], step=10.0
            )
        with col2:
            pan_y = st.number_input(
                "Pan Y", value=self.session.viz_state.pan_position[1], step=10.0
            )
