...
class MCTSVisualizer(TreeVisualizer):
    ...
    def visualize_mcts(
        ...
        with metrics_col2:
            ...
            confidences = [
                data["confidence"] for _, data in self._graph.nodes(data=True)
            ]
            if confidences:
    ...
    def _add_node_recursive(self, node: ThoughtNode):
        ...
        self._graph.add_node(node.id, confidence=node.confidence)
    ...
    def _add_nodes(self, fig: go.Figure):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=node_y,
                mode="markers",
                marker=dict(size=self.config.NODE_SIZE, color=node_color),
    ...
    def render(
        ...
        with container:
            ...
            if root_node is None:
                st.info(
                    "No exploration data available. Start a new exploration to visualize the reasoning process."
                )
                return
            ...
            with control_col2:
                ...
                min_confidence = st.slider(
                    "Min Confidence", min_value=0.0, max_value=1.0, value=0.3
                )

            # Create visualization container
            viz_container = st.container()
            self.visualizer.visualize_mcts(
                root_node=root_node, container=viz_container, title="MCTS Exploration"
            )
