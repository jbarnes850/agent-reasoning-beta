...
class GraphConfig:
    ...
    # Color schemes
    COLOR_SCHEMES = {
        ReasoningType.MCTS: {"node": "#1f77b4", "edge": "#7fcdbb", "text": "#2c3e50"},
...
class GraphVisualizer:
    ...
    def optimize_layout(self) -> None:
        ...
        self._node_positions = {
            node: (pos[0], pos[1]) for node, pos in self._layout.items()
        }
    ...
    async def visualize_agent_network(
        ...
        self._graph.clear()
        for agent in agents:
            self._graph.add_node(agent["id"], **agent)
        for interaction in interactions:
            self._graph.add_edge(interaction["from"], interaction["to"], **interaction)
    ...
    def _create_interaction_traces(self, interactions: List[Dict[str, Any]]) -> None:
        ...
        for interaction in interactions:
            ...
            # Edge styling
            color = self.config.COLOR_SCHEMES[AgentRole(interaction["type"])]["edge"]
    ...
    def _add_metrics_summary(
        self, fig: go.Figure, agents: List[Dict[str, Any]]
    ) -> None:
        ...
        total_interactions = sum(
            agent["metrics"]["total_interactions"] for agent in agents
        )
        avg_success_rate = (
            sum(agent["metrics"]["success_rate"] for agent in agents) / total_agents
        )
