...
from src.core.types import Evidence, ProofNode, VerificationChain
from src.visualization.components.views.shared.trees import TreeConfig, TreeVisualizer
...
class VerificationConfig(TreeConfig):
    ...
    # Node styling
    EVIDENCE_TYPES = {"direct": "#2ecc71", "indirect": "#f1c40f", "inferred": "#e74c3c"}
...
class VerificationVisualizer(TreeVisualizer):
    ...
    def _add_proof_tree(
        self, fig: go.Figure, chain: VerificationChain, row: int, col: int
    ):
        ...
        for node in graph.nodes():
            ...
            node_data.append(
                [str(node), attrs["type"], attrs["confidence"], len(attrs["evidence"])]
            )
        ...
        scatter = go.Scatter(
            ...
            y=node_y,
            mode="markers",
            marker=dict(size=node_size, color=node_color, showscale=False),
            text=node_text,
    ...
    def _build_proof_tree(self, node: ProofNode, graph: nx.DiGraph):
        """Build networkx graph from proof tree."""
        graph.add_node(
            node.id, type=node.type, confidence=node.confidence, evidence=node.evidence
        )
    ...
    def _add_evidence_map(
        ...
        for node_type in ["evidence", "claim"]:
            ...
            if node_type == "evidence":
                color = [
                    self.config.EVIDENCE_TYPES[graph.nodes[n]["subtype"]] for n in nodes
                ]
            else:
                color = [
    ...
    def _add_confidence_distribution(
        ...
        node_confidence = [node.confidence for node in chain.nodes.values()]
        evidence_confidence = [evidence.confidence for evidence in chain.evidence]

        # Create histograms
        nodes = go.Histogram(x=node_confidence, name="Nodes", opacity=0.75)

        evidence = go.Histogram(x=evidence_confidence, name="Evidence", opacity=0.75)
    ...
    def _add_verification_timeline(
        ...
        scatter = go.Scatter(
            ...
            marker=dict(
                size=10,
                color=[self._get_confidence_color(conf) for conf in confidences],
            ),
            text=statuses,
            name="Verification",
        )
...
class VerificationView:
    ...
    def render(
        ...
        with container:
            ...
            if chain is None:
                st.info(
                    "No verification data available. Start verification to analyze the reasoning process."
                )
                return
            ...
            with control_col2:
                ...
                show_confidence = st.checkbox(
                    "Show Confidence Distribution", value=True
                )
            ...
            self.visualizer.visualize_verification(
                chain=chain, container=viz_container, title="Verification Analysis"
            )
